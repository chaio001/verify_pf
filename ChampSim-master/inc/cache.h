...
//for pythia's use of find_if function
#include <algorithm>
// PAGE
...
class CACHE : public MEMORY {
  public:
    ...
    uint32_t l2c_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in),
         llc_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in, uint64_t instr_id, uint64_t curr_cycle),
    ...
    //functions added to port pythia
    ...
    void llc_prefetcher_broadcast_bw(uint8_t bw_level);

    void broadcast_ipc(uint8_t ipc);
    void l1d_prefetcher_broadcast_ipc(uint8_t ipc);
    void l2c_prefetcher_broadcast_ipc(uint8_t ipc);
    void llc_prefetcher_broadcast_ipc(uint8_t ipc);
    ...
    void l1d_prefetcher_broadcast_acc(uint32_t bw_level);
    void l2c_prefetcher_broadcast_acc(uint32_t bw_level);
    void llc_prefetcher_broadcast_acc(uint32_t bw_level);
    ...
    uint32_t llc_prefetcher_prefetch_hit(uint64_t addr, uint64_t ip, uint32_t metadata_in);



    //end of functions added to port pythia
