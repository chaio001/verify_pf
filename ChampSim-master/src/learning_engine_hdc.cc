...
LearningEngineHDC::LearningEngineHDC(Prefetcher* parent, float alpha, float gamma, float epsilon, uint32_t actions, uint32_t states, uint64_t seed, std::string policy, std::string type)
    : m_parent(parent), m_alpha(alpha), m_gamma(gamma), m_epsilon(epsilon)  // make it small, as true value indicates exploration
      ,
      m_actions(actions),
      m_states(states),
      m_seed(seed),
...
LearningType LearningEngineHDC::parseLearningType(std::string str) {
...
Policy LearningEngineHDC::parsePolicy(std::string str) {
