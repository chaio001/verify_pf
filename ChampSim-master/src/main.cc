...
uint64_t warmup_instructions = 1000000,
         simulation_instructions = 84756213,
         champsim_seed;
...
void finish_warmup() {
    ...
    for (uint32_t i = 0; i < NUM_CPUS; i++) {
        ...
        ooo_cpu[i].begin_sim_instr = 10000000;
    ...
    if (pr == page_table.end()) {  // no VA => PA translation found
        ...
        if (allocated_pages >= DRAM_PAGES) {  // not enough memory
            ...
            if (pr == page_table.end())
                assert(0);
#endif
            DP(if (warmup_complete[cpu]) { cout << "[SWAP] update page table NRU_vpage: " << hex << pr->first << " new_vpage: " << vpage << " ppage: " << pr->second << dec << endl; });
...
int main(int argc, char** argv) {
    ...
    cout << "dram channel width " << DRAM_CHANNEL_WIDTH << endl;
    ...
    printf("dram_dbus_return_time %d = block size %d DRAM_CHANNEl_WIDTH %d, CPU_FREQ %d, DRAM_MTPS %d\n", DRAM_DBUS_RETURN_TIME, BLOCK_SIZE, DRAM_CHANNEL_WIDTH, CPU_FREQ, DRAM_MTPS);
    ...
    cout << "after dram_dbus_max_cas" << endl;
    ...
    while (run_simulation) {
        ...
        for (int i = 0; i < NUM_CPUS; i++) {
            ...
            */
            ...
            // simulation complete
            if ((all_prefetch_warmup_complete > NUM_CPUS) && (simulation_complete[i] == 0) && (ooo_cpu[i].num_retired >= (ooo_cpu[i].begin_sim_instr + ooo_cpu[i].simulation_instructions))) {
