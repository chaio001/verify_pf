...
#include "champsim.h"
#include "hdc_feature_knowledge.h"
#include "hdc_util.h"
...
extern bool hdcpf_enable_pt_address_compression;
...
extern bool hdcpf_enable_dyn_degree_detector;
...
uint32_t State::value() {
    uint64_t value = 0;
    switch (knob::hdcpf_state_type) {
        case 1: /* Only PC */
            // return (uint32_t)(pc % knob::hdcpf_max_states);
            value = pc;
            break;
        ...
        case 2: /* PC+ offset */
            ...
            value = value << 6;
            value = value + offset;
            // value = value % knob::hdcpf_max_states;
            // return value;
            break;
    ...
    assert(hashed_value < knob::hdcpf_max_states);
...
uint32_t State::get_hash(uint64_t key) {
    uint32_t value = 0;
    switch (knob::hdcpf_state_hash_type) {
        case 1: /* simple modulo */
            value = (uint32_t)(key % knob::hdcpf_max_states);
            break;

        case 2:
            value = HashZoo::jenkins((uint32_t)key);
            value = (uint32_t)(value % knob::hdcpf_max_states);
            break;
...
void HDCPF_STEntry::update(uint64_t page, uint64_t pc, uint32_t offset, uint64_t address) {
    assert(this->page == page);

    /* insert PC */
    if (this->pcs.size() >= knob::hdcpf_max_pcs) {
        this->pcs.pop_front();
    }
    ...
    if (!this->offsets.empty()) {
        ...
        if (this->deltas.size() >= knob::hdcpf_max_deltas) {
    ...
    /* insert offset */
    if (this->offsets.size() >= knob::hdcpf_max_offsets) {
    ...
    if (knob::hdcpf_print_trace) {
        fprintf(stdout, "[TRACE] page %16lx| pc %16lx| offset %2u\n", page, pc, offset);
    }
}

uint32_t HDCPF_STEntry::get_delta_sig() {
...
uint32_t HDCPF_STEntry::get_delta_sig2() {
    uint32_t curr_sig = 0;
...
uint32_t HDCPF_STEntry::get_pc_sig() {
    uint32_t signature = 0;
...
uint32_t HDCPF_STEntry::get_offset_sig() {
    uint32_t signature = 0;
...
void HDCPF_STEntry::track_prefetch(uint32_t pred_offset, int32_t pref_offset) {
...
void HDCPF_STEntry::insert_action_tracker(int32_t pref_offset) {
    // bool found = false;
    ...
    if (it != action_tracker.end()) {
        ...
        if (action_tracker.size() >= knob::hdcpf_action_tracker_size) {
...
bool HDCPF_STEntry::search_action_tracker(int32_t action, int32_t& conf) {
    conf = 0;
...
void HDCPFRecorder::record_access(uint64_t pc, uint64_t address, uint64_t page, uint32_t offset, uint8_t bw_level) {
...
void HDCPFRecorder::record_trigger_access(uint64_t page, uint64_t pc, uint32_t offset) {
    unique_trigger_pcs.insert(pc);
}

void HDCPFRecorder::record_access_knowledge(HDCPF_STEntry* stentry) {
...
void print_access_debug(HDCPF_STEntry* stentry) {
    bool to_print = true;
    if (knob::hdcpf_print_access_debug_pc) {
        if (stentry->unique_pcs.find(knob::hdcpf_print_access_debug_pc) == stentry->unique_pcs.end()) {
            to_print = false;
        } else {
            debug_print_count++;
            if (debug_print_count > knob::hdcpf_print_access_debug_pc_count) {
                to_print = false;
            }
        }
    }
