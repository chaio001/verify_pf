...
def train_command():
    ...
    parser.add_argument('--debug-amplitude', type=int, default=None)

    args = parser.parse_args(sys.argv[2:])

    train_data, eval_data = read_load_trace_data(args.load_trace, args.num_prefetch_warmup_instructions)

    if args.debug_amplitude is not None:
        model = Model(debug_amplitude=args.debug_amplitude)
    else:
        model = Model()
